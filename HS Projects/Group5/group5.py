...
# ── 3. RESET────
def reset():
    global lp_y, rp_y, bx, by, bvx, bvy, Paddle_L_H, Paddle_R_H,FPS_add
    bx, by = W / 2, H / 2
    bvx = bvy = 0
    lp_y = (H - PADDLE_H+Paddle_R_H) // 2
    rp_y = (H - PADDLE_H+Paddle_L_H) // 2
...
def update(keys):
    ...
    global l_score, r_score, state, Paddle_L_H, Paddle_R_H, FPS_add, R,G,B, RGB, Turn
    if R < 255 and Turn == "R":
        R+=5
    elif R >= 255 and Turn == "R": 
        Turn = "G"
    if G < 255 and Turn == "G":
        R-=5
        G+=5
    elif G >= 255 and Turn == "G":
        Turn = "B"
        R = 0
    if B < 255 and Turn == "B":
        G-=5
        B+=5
    elif B >= 255 and Turn == "B":
        Turn = "R"
        R = 0
        G = 0
        B = 0
    ...
    if (bvx < 0
            ...
            and lp_y <= by <= lp_y + PADDLE_H + Paddle_L_H):
    ...
    if (bvx > 0
            ...
            and rp_y <= by <= rp_y + PADDLE_H + Paddle_R_H):
...
def draw():
    ...
    # Paddles & ball
    pygame.draw.rect(
        screen, RED , (INSET, lp_y, PADDLE_W, Paddle_L_H + PADDLE_H), border_radius=4
    )
    pygame.draw.rect(
        screen, BLUE,
        (W - INSET - PADDLE_W, rp_y, PADDLE_W, Paddle_R_H + PADDLE_H), border_radius=4
    )
    pygame.draw.circle(screen, RGB, (int(bx), int(by)), BALL_SIZE // 2)
