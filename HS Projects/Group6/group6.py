...
diff = str.lower(input("what difficulty do you want? (normal / n, hard / h, impossible / i) "))
...
def AI_n():
    ...
    if rp_y + 45 > by and rp_y > 0:
        for i in range(move_i):
            rp_y-= .5
    if rp_y + 45 < by and rp_y < H - PADDLE_H:
        for i in range(move_i):
            rp_y += .5
...
def update(keys):
    ...
    if (keys[pygame.K_w] or keys[pygame.K_UP]) and lp_y > 0:
        lp_y -= PADDLE_SPEED
    if (keys[pygame.K_s] or keys[pygame.K_DOWN]) and lp_y < H - PADDLE_H:
        lp_y += PADDLE_SPEED
...
def draw():
    ...
    # Control hints
    screen.blit(font_hint.render("Left: W or up / S or down", True, GREY), (20, H - 20))
    rh = font_hint.render(f"AI.{diff}", True, GREY)
